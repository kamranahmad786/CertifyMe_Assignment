...
// closeOpportunityModal defined below with opportunity CRUD
...
document.getElementById('opportunityForm').addEventListener('submit', function(e) {
    ...
    const url = currentEditId ? API+'/api/opportunities/'+currentEditId : API+'/api/opportunities';
    const method = currentEditId ? 'PUT' : 'POST';

    fetch(url,{method,headers:{'Content-Type':'application/json'},credentials:'include',
        body:JSON.stringify({name,duration,start_date:startDate,description,skills:skillsRaw,category,future_opportunities:futureOpportunities,max_applicants:maxApplicants||null})
    }).then(r=>r.json()).then(res=>{
        if(res.success){
            showToast(res.message);
            closeOpportunityModal();
            this.reset(); currentEditId=null;
            document.querySelector('#opportunityModal .modal-header h3').textContent='Add New Opportunity';
            loadOpportunities();
        } else { showToast(res.message||'Error saving opportunity'); }
...
function loadOpportunities() {
    fetch(API+'/api/opportunities',{credentials:'include'}).then(r=>r.json()).then(res=>{
        const grid = document.querySelector('.opportunities-grid');
        if(!grid) return;
        grid.innerHTML = '';
        if(!res.success || !res.opportunities.length){
            grid.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--qf-text-light);"><svg viewBox="0 0 24 24" style="width:48px;height:48px;stroke:var(--qf-text-light);fill:none;stroke-width:1.5;margin-bottom:12px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><h4 style="margin-bottom:8px;">No Opportunities Yet</h4><p>Click \"Add New Opportunity\" to create your first opportunity.</p></div>';
            return;
        }
            ...
            card.innerHTML = `
                ...
                <div class="opportunity-footer">
                    ...
                    <div style="display:flex;gap:8px;">
                        <button class="view-course-btn" style="width:auto;padding:8px 16px;" onclick="viewOpp(${opp.id})">View Details</button>
                        <button class="view-course-btn" style="width:auto;padding:8px 16px;background:var(--qf-mint);color:#fff;" onclick="editOpp(${opp.id})">Edit</button>
                        <button class="view-course-btn" style="width:auto;padding:8px 16px;background:#e74c3c;color:#fff;" onclick="deleteOpp(${opp.id})">Delete</button>
                    </div>
                </div>`;
            grid.appendChild(card);
        });
        window._opportunities = res.opportunities;
    }).catch(()=>{});
}

function viewOpp(id) {
    const opp = (window._opportunities||[]).find(o=>o.id===id);
    if(!opp) return;
    const skills = Array.isArray(opp.skills)?opp.skills:opp.skills.split(',').map(s=>s.trim());
    openOpportunityDetails(opp.name, {duration:opp.duration,startDate:opp.start_date,description:opp.description,skills,applicants:opp.max_applicants||0,futureOpportunities:opp.future_opportunities,prerequisites:''});
}
...
function deleteOpp(id) {
    ...
    fetch(API+'/api/opportunities/'+id,{method:'DELETE',credentials:'include'}).then(r=>r.json()).then(res=>{
        if(res.success){ showToast(res.message); loadOpportunities(); }
        else showToast(res.message||'Error deleting');
    }).catch(()=>showToast('Server error'));
}
...
document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    const remember = document.querySelector('#loginForm .remember-me input[type=checkbox]').checked;
    ...
    fetch(API+'/api/login',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',
        body:JSON.stringify({email,password,remember})
    }).then(r=>r.json()).then(res=>{
        if(res.success){
            showToast('Login successful! Redirecting...');
            setTimeout(()=>showDashboard(res.admin),1200);
        } else {
            showError('loginPasswordErr',res.message||'Invalid email or password');
            shakeForm('loginForm');
        }
        generateCaptcha('login');
    }).catch(()=>{ showError('loginPasswordErr','Server error. Please try again.'); shakeForm('loginForm'); generateCaptcha('login'); });
});
...
document.getElementById('signupForm').addEventListener('submit', function(e) {
    ...
    const form = this;
    fetch(API+'/api/signup',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({full_name:name,email,password,confirm_password:confirmPassword})
    }).then(r=>r.json()).then(res=>{
        if(res.success){
            showToast('Account created successfully!');
            generateCaptcha('signup');
            form.reset(); checkStrength('');
            setTimeout(()=>showPage('loginPage'),1500);
        } else {
            showError('signupEmailErr',res.message||'Registration failed');
            shakeForm('signupForm'); generateCaptcha('signup');
        }
    }).catch(()=>{ showError('signupEmailErr','Server error. Please try again.'); shakeForm('signupForm'); generateCaptcha('signup'); });
});
...
document.getElementById('forgotForm').addEventListener('submit', function(e) {
    ...
    const form = this;
    fetch(API+'/api/forgot-password',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({email})
    }).then(r=>r.json()).then(res=>{
        showToast(res.message||'Reset link sent to your email!');
        generateCaptcha('forgot'); form.reset();
    }).catch(()=>{ showToast('Reset link sent to your email!'); generateCaptcha('forgot'); form.reset(); });
});
